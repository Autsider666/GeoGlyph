...
export class ColliderHelper {
    public static generateShapePath(collider: Collider): Path2D {
        if (collider instanceof CircleCollider) {
            return CanvasHelper.generateCirclePath(collider.center, collider.radius);
        } else if (collider instanceof PolygonCollider) {
            return CanvasHelper.generatePolygonPath(this.getPolygonColliderPoints(collider));
        } else if (collider instanceof CompositeCollider) {
            return CanvasHelper.generatePolygonPath(this.getCompositeColliderPoints(collider));
        } else {
